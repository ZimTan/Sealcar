...
def squeezenet_model(input_shape, memory, num_outputs):
  ...
  inputs = Input(shape=(memory, input_shape[0], input_shape[1]))
  ...
  avg_pool = Dense(40, activation='tanh')(avg_pool)
  outputs = Dense(num_outputs, activation='tanh')(avg_pool)

  model = Model(inputs=inputs, outputs=outputs)
  model.ch_order = 'channels_first'
